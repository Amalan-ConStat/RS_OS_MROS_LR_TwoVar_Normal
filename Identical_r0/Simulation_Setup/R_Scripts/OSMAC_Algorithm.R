...
AlgTwoStp <- function(r1=r1, r2=r2,Y,X,n,Real_Data,alpha,combs,All_Covariates){
    ...
    for (i in 1:length(r2)) 
    {
      ...
      fit_Assumed_New.mVc <-lapply(1:length(combs), function(a){
        getMLE(x=x_Assumed.mVc[[a]], y=y_Assumed.mVc[[a]], 
               w=c(1 / PI_Assumed_New.mVc[[a]], pinv.prop)) # Assumed Data New probabilities
      }) 
      ...
      pi<- lapply(1:length(combs),function(a){
        invlogit(x_Assumed.mVc[[a]] %*% beta.mVc_New[[a]][i,])
      })
      W<-lapply(1:length(combs),function(a){
        diag(as.vector(pi[[a]]*(1-pi[[a]])*c(1 / PI_Assumed_New.mVc[[a]], pinv.prop)))
      }) 
      Mx<-lapply(1:length(combs),function(a){
        solve((t(x_Assumed.mVc[[a]]) %*% W[[a]] %*% x_Assumed.mVc[[a]]))
      })
      Middle<-lapply(1:length(combs),function(a){
        diag(((as.vector(y_Assumed.mVc[[a]])-as.vector(pi[[a]]))*
                as.vector(c(1 / PI_Assumed_New.mVc[[a]], pinv.prop)))^2)
      })
      V_Temp<-lapply(1:length(combs),function(a){
        (t(x_Assumed.mVc[[a]])%*%Middle[[a]]%*%x_Assumed.mVc[[a]])
      })
      ...
      fit_Assumed_New.mMSE <- lapply(1:length(combs),function(a){
        getMLE(x=x_Assumed.mMSE[[a]], y=y_Assumed.mMSE[[a]], 
               w=c(1 /PI_Assumed_New.mMSE[[a]], pinv.prop)) # Assumed Data New probabilities
      })
      ...
      # Assumed Data New probabilities
      pi<- lapply(1:length(combs),function(a){
        invlogit(x_Assumed.mMSE[[a]] %*% beta.mMSE_New[[a]][i,])
      }) 
      W<-lapply(1:length(combs),function(a){
        diag(as.vector(pi[[a]]*(1-pi[[a]])*c(1 / PI_Assumed_New.mMSE[[a]], pinv.prop)))
      }) 
      Mx<-lapply(1:length(combs),function(a){
        solve((t(x_Assumed.mMSE[[a]]) %*% W[[a]] %*% x_Assumed.mMSE[[a]]))
      })
      Middle<-lapply(1:length(combs),function(a){
        diag(((as.vector(y_Assumed.mMSE[[a]])-as.vector(pi[[a]]))*
                as.vector(c(1 / PI_Assumed_New.mMSE[[a]], pinv.prop)))^2)
      })
      V_Temp<-lapply(1:length(combs),function(a){
        (t(x_Assumed.mMSE[[a]])%*%Middle[[a]]%*%x_Assumed.mMSE[[a]])
      }) 
