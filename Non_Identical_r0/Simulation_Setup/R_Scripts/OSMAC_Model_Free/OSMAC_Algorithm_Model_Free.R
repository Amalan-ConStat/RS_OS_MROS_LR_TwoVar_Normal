...
OSMAC_MF <- function(r1,r2,Y,X,n,alpha,combs,All_Covariates,Theta) {
  ...
  beta.prop<-lapply(1:length(combs),function(i) {
    getMLE(x=X[idx.prop,colnames(X) %in% combs[[i]]  ], y=y.prop, w=pinv.prop)})
   
  for (i in 1:length(combs)) 
  {
    if (anyNA(beta.prop[[i]]$par))
    {
  ...
  P.prop<-lapply(1:length(combs), function(i)
    1 - 1 / (1 + exp(X[,colnames(X) %in% combs[[i]]] %*% beta.prop[[i]]$par)))
  ...
  for (i in 1:length(r2)) 
  {
    ...
    for (j in 1:length(combs))
    {
      ...
      Mx <- solve(Mx) #
      Middle <- diag(((as.vector(Y[c(idx.mVc, idx.prop)])-as.vector(pi))*as.vector(c(1 / pjoin.mVc[idx.mVc], pinv.prop)))^2) #
    ...
    for (j in 1:length(combs)) 
    {
      ...
      Mx <- solve(Mx) #
      Middle <- diag(((as.vector(Y[c(idx.mMSE, idx.prop)])-as.vector(pi))*as.vector(c(1 / pjoin.mMSE[idx.mMSE], pinv.prop)))^2) #
